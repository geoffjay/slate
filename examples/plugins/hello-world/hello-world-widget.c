...
G_DEFINE_FINAL_TYPE (HelloWorldWidget, hello_world_widget, GTK_TYPE_WIDGET)
...
on_change_button_clicked (GtkButton *button, gpointer user_data)
{
  ...
  if (!self->editing) {
    ...
    gtk_editable_set_text (GTK_EDITABLE (self->greeting_entry), self->greeting);
    gtk_widget_grab_focus (GTK_WIDGET (self->greeting_entry));
    gtk_button_set_label (button, "Save");
    self->editing = TRUE;
  } else {
    /* Save and switch back to display mode */
    const char *new_greeting = gtk_editable_get_text (GTK_EDITABLE (self->greeting_entry));
...
hello_world_widget_init (HelloWorldWidget *self)
{
  ...
  self->editing = FALSE;
    ...
    /* Create content box as the main widget */
  ...
  /* Set the content box as the child of this widget */
  gtk_widget_set_parent (GTK_WIDGET (self->content_box), GTK_WIDGET (self));
